    ...
    <div style="font-family:'Inter',sans-serif">
      ...
      <div class="rounded-2xl mb-6 overflow-hidden" style="background:#141414; border:1px solid #262626">
        ...
        <div class="flex flex-wrap gap-2 p-4">
          ...
          <span *ngIf="!sheets.length" class="text-xs text-textGray px-2 py-2">No sheets available yet.</span>
        </div>
      </div>
      ...
      <div *ngIf="!loadingCols && columns.length" class="rounded-2xl p-5 mb-6" style="background:#141414; border:1px solid #262626">
        ...
        <div class="flex items-center gap-4 mt-6 flex-wrap">
          ...
          <button *ngIf="isAdmin" class="btn-export" (click)="exportExcel()">
      ...
      <div *ngIf="!loadingCols && !columns.length" class="flex flex-col items-center py-20 text-center">
        ...
        <p class="text-sm text-textGray max-w-xs">{{ isAdmin ? 'Upload a spreadsheet first to define the columns.' : 'Ask your admin to upload data to this sheet.' }}</p>
      </div>
...
export class EntryComponent implements OnInit, OnDestroy {
  ...
  selectSheet(name: string) { this.ds.setActiveSheet(name); }
  ...
  exportExcel() {
    ...
    this.http.get(url, { headers, responseType: 'blob' }).subscribe({
      ...
      error: () => { this.errorMsg = 'Export failed.'; setTimeout(() => this.errorMsg = '', 4000); }
    });
  }
  ...
  createSheet() {
    ...
    this.ds.createSheet(this.newSheetName.trim()).subscribe({
      next: () => { this.creatingSheet = false; this.showNewSheet = false; this.newSheetName = ''; this.ds.loadSheets(); },
