    ...
    <div class="doc" *ngIf="data" id="ins-doc-main">
      ...
      <table class="grid-table">
        <tr>
          ...
          <td class="val" [attr.colspan]="generatedByName && generatedByName !== (agentName || '') ? 1 : 3">
            {{ agentName || '—' }}
          </td>
          <ng-container *ngIf="generatedByName && generatedByName !== (agentName || '')">
            <td class="lbl">Generated By</td>
            <td class="val">{{ generatedByName }}</td>
          </ng-container>
        </tr>
        <tr>
          <td class="lbl">Date &amp; Time</td>
          <td class="val" colspan="3">{{ formatDate(generatedAt) }}</td>
        </tr>
      </table>
      ...
      <div style="margin-top: 15px; font-size: 9px; color: #555; line-height: 1.5;">
        ...
        <div>Note: Premium has been calculated for
          <strong>{{ agentName || generatedByName || '—' }}</strong>
          <span *ngIf="generatedByName && generatedByName !== (agentName || '')"> (via {{ generatedByName }})</span>
        </div>
