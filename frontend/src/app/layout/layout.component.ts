    ...
    <div class="flex h-screen bg-background overflow-hidden" style="font-family:'Inter',sans-serif">
      ...
      <aside [class.translate-x-0]="menuOpen" [class.-translate-x-full]="!menuOpen"
        ...
        <nav class="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
          ...
          <a *ngFor="let item of navItems"
            ...
            routerLinkActive="!text-textLight"
            [routerLinkActiveOptions]="{exact:false}"
            (click)="menuOpen=false"
            class="flex items-center gap-3 px-3 py-2.5 text-textGray hover:text-textLight rounded-xl transition-all text-sm font-medium group"
        ...
        <div class="p-4" style="border-top:1px solid #262626">
          <div class="flex items-center gap-3 mb-4 px-2">
            ...
            <div class="overflow-hidden">
              ...
              <p class="text-[10px] text-textGray truncate">Logged in</p>
            </div>
          </div>
          <button (click)="logout()" 
      ...
      <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header class="hidden md:flex items-center justify-between px-8 py-4"
          ...
          <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg" style="background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.2)">
...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  private meta: Record<string, {title:string;sub:string}> = {
    ...
    '/app/search':    { title:'Search Vehicle', sub:'Lookup by plate number' },
